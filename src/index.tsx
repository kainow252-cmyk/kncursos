...
// Processar pagamento e registrar venda com PagBank
app.post('/api/sales', async (c) => {
  const { DB, RESEND_API_KEY, EMAIL_FROM, PAGBANK_TOKEN, PAGBANK_ENV, PAGBANK_PUBLIC_KEY } = c.env
  ...
  // Processar pagamento via PagBank
  console.log('='.repeat(50))
  console.log('[PAGAMENTO] Processando via PagBank')
  console.log('='.repeat(50))
  ...
  if (!pagbankResult.success) {
    // PagBank falhou
    console.error('[PAGBANK] ❌ Erro ao processar pagamento:', pagbankResult.error)
    
    return c.json({ 
      error: 'Não foi possível processar o pagamento. Tente novamente mais tarde.',
      details: pagbankResult.error
    }, 500)
  }
  
  // Pagamento aprovado via PagBank
  const paymentSuccess = true
  const paymentGateway = 'pagbank'
  const paymentId = pagbankResult.payment_id
  const customerId = pagbankResult.customer_id
